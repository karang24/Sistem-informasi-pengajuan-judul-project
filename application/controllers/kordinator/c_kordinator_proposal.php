...
class C_kordinator_proposal extends CI_Controller {
	...
	function updatedata($id_proposal){
        if($_POST==NULL){
			...
			$data['hasil1'] = $this->m_kordinator_proposal->bacadata3();
			$data['hasil2'] = $this->m_kordinator_proposal->bacastatus();
	  ...
	  function tampil33($npm){
	if($this->input->post('submit')){
            $this->load->model('m_kordinator_proposal');
			$data['hasil1'] = $this->m_kordinator_proposal->bacadata();
			
			$data['hasil4'] = $this->m_kordinator_proposal->filterdatamhs($npm);
            $this->m_kordinator_proposal->tambahrevisi();
            redirect('kordinator/c_kordinator_proposal/tampilrevisi');
        }
		$this->load->model('m_kordinator_proposal');
		
		$data['hasil4'] = $this->m_kordinator_proposal->filterdatamhs($npm);
        $this->load->view('kordinator/v_revisi',$data);
    }
	function tampil(){
        $this->load->model('m_kordinator_proposal');
